//
//  NicknameViewController 2.swift
//  Offroad-iOS
//
...
final class ReNicknameViewController: UIViewController {
    ...
    private func setupBindings() {
        ...
        nicknameView.textField.rx.controlEvent([.editingDidBegin])
            .subscribe(onNext: { [weak self] in
                self?.nicknameView.textField.layer.borderColor = UIColor.main(.main2).cgColor
            })
            .disposed(by: disposeBag)
        
        nicknameView.textField.rx.controlEvent([.editingDidEnd])
            .subscribe(onNext: { [weak self] in
                let isTextFieldEmpty = self?.nicknameView.textField.text?.isEmpty ?? true
                self?.nicknameView.textField.layer.borderColor = isTextFieldEmpty ? UIColor.grayscale(.gray100).cgColor : UIColor.main(.main2).cgColor
            })
            .disposed(by: disposeBag)
...
extension ReNicknameViewController: UITextFieldDelegate {
